...
typedef struct{
	float OUT1;					//OUT1接口电流
	float OUT2;					//OUT2接口电流
	float TypeC;				//TypeC接口电流
	float VIN;					//VIN(安卓口)接口电流
}Current;

extern Current current;			//各接口电流结构体

typedef struct{
	unsigned char OUT1;			//OUT1接口电压
	unsigned char OUT2;			//OUT2接口电压
	unsigned char TypeC;		//TypeC接口电压
	unsigned char VIN;			//VIN(安卓口)接口电压
}Voltage;

extern Voltage voltage;			//各接口电压结构体

typedef struct{
	unsigned char OUT1;		   	//OUT1接口开启状态
	unsigned char OUT2;		   	//OUT2接口开启状态
	unsigned char TypeC;	   	//TypeC接口开启状态
	unsigned char VIN;		   	//VIN(安卓口)开启状态
}Mos_state;

extern Mos_state mos_state;	    //各接口开启状态结构体
