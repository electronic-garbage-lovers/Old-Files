...
l03
	...
	ldr r0,cyclesperscanline	;每个扫描行的周期
	ldr r1,frame
	tst r1,#1
	subeq r0,r0,#CYCLE			;每帧有1/3的CPU周期
...
line242NMI ;---------------------------
	...
	str r0,frame
 [ DEBUG
	mov r1,#REG_BASE			;darken screen during NES vblank在NES VBLANK屏幕变暗
	mov r0,#0x00f1
...
;----------------------------------------------------------
	tst cycles,#CYC_I
	bne default_scanlinehook		;we dont want no stinkin irqs我们不希望没有发臭的IRQ
;----------------------------------------------------------
irq6502
;----------------------------------------------------------
	ldr r12,=IRQ_VECTOR
...
;	bic cycles,cycles,#CYC_D	;and decimal mode 和十进制模式

	ldr r0,memmap_tbl+7*4
	ldrb m6502_pc,[r0,r12]!
	ldrb r2,[r0,#1]
	orr m6502_pc,m6502_pc,r2,lsl#8
	encodePC				;get IRQ vector得到IRQ向量
