...
u8 *textstart;//第一NES ROMpoints to first NES rom (initialized by boot.s)
...
//		pogosize=magptr[8];							//file size
	...
	*MEM_PALETTE=0;					//black background (avoids blue flash when doing multiboot)
	...
	lzo_init();	//init compression lib for savestates 初始化压缩库即时存档
...
void rommenu(void) {
	...
	if(pogoshell)
	{
		loadcart(0,g_emuflags&0x304);		//Also save country	还可以节省国家
		get_saved_sram();
	}
	else
	{
		...
		if(romz>1){
			i=drawmenu(sel);
			loadcart(sel,i|(g_emuflags&0x300));  //(keep old gfxmode)保留旧的GFX模式
			get_saved_sram();
			lastselected=sel;
			for(i=0;i<8;i++)
			{
